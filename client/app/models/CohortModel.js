            ...
            'exome': [
                {
                    ...
                    'tbi': this.demoVcf.exome + '.tbi',
                    'bam': this.demoBams.exome['proband'],
                    'bai': this.demoBams.exome['proband'] + '.crai'
                },
                {
                    relationship: 'mother',
                    ...
                    'bai': this.demoBams.exome['mother'] + '.crai'
                },
                {
                    relationship: 'father',
                    affectedStatus: 'unaffected',
                    ...
                    'bai': this.demoBams.exome['father'] + '.crai'
                },
                {
                    ...
                    'vcf': this.demoVcf.exome,
                    'tbi': this.demoVcf.exome + '.tbi',
                    'bam': this.demoBams.exome['sibling'],
                    'bai': this.demoBams.exome['sibling'] + '.crai'
                },
            ],
            'genome': [
                {
                    ...
                    'tbi': this.demoVcf.genome + '.tbi',
                    'bam': this.demoBams.genome['proband'],
                    'bai': this.demoBams.genome['proband'] + '.crai'
                },
                {
                    relationship: 'mother',
                    affectedStatus: 'unaffected',
                    ...
                    'bai': this.demoBams.genome['mother'] + '.crai'
                },
                {
                    relationship: 'father',
                    affectedStatus: 'unaffected',
                    ...
                    'bai': this.demoBams.genome['father'] + '.crai'
                },
            ]
        }
...
//todo: refactor to handle all cases of duo, not just a missing parent
    ...
    _promiseLoadSfariVariants(theGene, theTranscript) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            self.sampleMap['sfari-variants'].model.promiseAnnotateSfariVariants(theGene, theTranscript, [self.sampleMap['sfari-variants'].model], {
                'isMultiSample': false,
                'isBackground': false
            })
                .then(function (resultMap) {
    ...
    promiseLoadVariants(theGene, theTranscript, options) {
        ...
        return new Promise(function (resolve, reject) {
            let theOptions = $.extend({
                'isMultiSample': self.mode == 'trio' && self.samplesInSingleVcf(),
                'isBackground': false,
                'noAnnotationMode': false,
            }, options);
    ...
    promiseAnnotateVariants(theGene, theTranscript, options = {}) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (options.isMultiSample) {
                ...
                let probandModel = self.sampleMap['proband'].model;

                let p = new Promise((subResolve, subReject) => {
                    probandModel.promiseFetchVariantCount(theGene, theTranscript, options)
                        .then(variantCount => {
                            if (+variantCount > self.VARIANT_LIMIT) {
                                console.log('Exceeded variant limit - only pulling back coding regions');
                                options.noAnnotationMode = true;
                                options.analyzeCodingVariantsOnly = false;
                            }
                            probandModel.promiseAnnotateVariants(theGene, theTranscript, self.getCanonicalModels(), options)
                                .then(resultMap => {
                                    theResultMap = resultMap;
                                    if (options.noAnnotationMode) {
                                        self.promiseFillCodingAnnotations(theGene, theTranscript, options);
                                    }
                                    if (!options.isBackground) {
                                        self.getCanonicalModels().forEach(function (model) {
                                ...
                                console.log('Something went wrong annotating variants: ' + error);
                                subReject(error);
                            });
                        }).catch(error => {
                        console.log('Something went wrong fetching variant count: ' + error);
                        subReject(error);
                    });
                });
                annotatePromises.push(p);
            } else {
                // Check if any of our samples exceed our variant limit
                let varCountPs = [];
                let totalCount = 0;
                for (var rel in self.sampleMap) {
                    let model = self.sampleMap[rel].model;
                    if (model.isVcfReadyToLoad() || model.isLoaded()) {
                        ...
                        if (rel !== 'known-variants' && rel !== 'sfari-variants') {
                            let p = new Promise((subresolve, subreject) => {
                                model.promiseFetchVariantCount(theGene, theTranscript, [model], options)
                                    .then(variantCount => {
                ...
                Promise.all(varCountPs)
                    .then(() => {
                        if (totalCount > self.VARIANT_LIMIT) {
                            options.analyzeCodingVariantsOnly = false;
                            options.noAnnotationMode = true;
                        }
                        for (var rel in self.sampleMap) {
                            ...
                            if (model.isVcfReadyToLoad() || model.isLoaded()) {
                                ...
                                if (rel !== 'known-variants' && rel !== 'sfari-variants') {
                                    let p = model.promiseAnnotateVariants(theGene, theTranscript, [model], options)
                                        .then(function (resultMap) {
            ...
            // todo: problem is that when we're resolving here, we don't wait until after the multiple promise resolve above is done
    ...
    /* Performs backend call to annotate coding variants and tells front-end to fill in retrieved annotations.
     * Called after a whole genome variant has been performed to fill in annotations */
    promiseFillCodingAnnotations(theGene, theTranscript, options) {
        const self = this;
        options.noAnnotationMode = false;
        options.analyzeCodingVariantsOnly = true;
        let probandModel = self.sampleMap['proband'].model;
        probandModel.promiseAnnotateVariants(theGene, theTranscript, self.getCanonicalModels(), options)
            .then(resultMap => {
                // todo: send annotation info to view to update
                // todo: send notice to user that not annotating variants
                console.log('UPDATE CODING ANNOTATIONS');
                console.log(resultMap);
            }).catch(err => {
                console.log('Problem with coding only annotate variants call: ' + err);
            });
    }
    ...
    organizeVariantsByFilterAndGene(activeFilterName, isFullAnalysis, interpretationFilters, variant, options = {
